    ...
    <div :class="wrapClasses" :style="styles">
        <div :class="classes">
            ...
            <div :class="[prefixCls + '-body']" :style="bodyStyle" ref="body" @scroll="handleBodyScroll"
                show="!((!!localeNoDataText && (!data || data.length === 0)) || (!!localeNoFilteredDataText && (!rebuildData || rebuildData.length === 0)))">
                <table-body
                    ref="tbody"
                    :prefix-cls="prefixCls"
                    :styleObject="tableStyle"
            ...
            <div :class="[prefixCls + '-footer']" v-if="pagination" ref="footer">
                <slot name="footer">
                    <Page :total="40" size="small" show-elevator show-sizer show-total placement="top"></Page>
                </slot>
            </div>
        </div>
    </div>
                ...
                this.$nextTick(() => {
                    ...
                    this.$nextTick(() => {
                        ...
                        if (this.data.length) {
                            ...
                            var noWidthColumnIndexs=[];
                            var totalWidth=0;
                            var contentWidth=parseInt((0, _assist.getStyle)(_this.$el, 'width'));
                            for(var i=0;i<_this.cloneColumns.length;i++){
                                if(!_this.cloneColumns[i].width){
                                    noWidthColumnIndexs.push(i);
                                }else{
                                    columnsWidth[i] = {
                                        width: _this.cloneColumns[i].width
                                    };
                                    totalWidth=totalWidth+_this.cloneColumns[i].width;
                                }
                            }
                            if(totalWidth<contentWidth && (totalWidth+noWidthColumnIndexs.length*50)<contentWidth){
                                var blankWidth=contentWidth-totalWidth;
                                var oneWidth=parseInt(blankWidth/noWidthColumnIndexs.length);
                                for(var i in noWidthColumnIndexs){
                                    if(i==noWidthColumnIndexs.length-1){
                                        columnsWidth[noWidthColumnIndexs[i]] = {
                                            width: blankWidth
                                        }
                                    }else{
                                        columnsWidth[noWidthColumnIndexs[i]] = {
                                        ...
                                        blankWidth=blankWidth-oneWidth
                                    }
                                }
                            }else{
                                for(var i in noWidthColumnIndexs){
